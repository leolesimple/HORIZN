...
- **KeyUsageService** — suivi d'usage des clés. `record(key)` accumule un delta en mémoire (`_pending`), `flush()` le reverse dans `data/api_keys.json` (relecture + merge + write atomique tmp/rename ; retire du delta uniquement les clés fichier réécrites, garde les clés `.env`). `start()`/`stop()` = timer de flush 30 s (`unref`), appelés par `index.js` (boot + shutdown). `report()` = inventaire masqué + `rateLimit.getUsage()` pour `/admin/keys`. **`_pending` doit rester `let`** (flush repose sur la ré-affectation, pas `.clear()`).
